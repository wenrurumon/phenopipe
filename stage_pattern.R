...
p.clust <- fc2(p.cca)
plotclust2(p.clust)
plotclust(p.clust$network,p.clust$cluster)
...
rlt[,1] <- 0
rlt <- rlt[match(names(sort(rlt[,4])),rownames(rlt)),]
...
par(mfrow=c(3,3))
for(i in 1:9){
  plot(rlt[i,],ylim=c(0,5),ylab='',xlab='',axes=F,main=rownames(rlt)[i],col=2)
  lines(rlt[i,])
}
par(mfrow=c(1,1))
...
x.pclust <- sapply(unique(p.clust$cluster),function(i){
  qpca(do.call(cbind,p[p.clust$cluster==i]),rank=1)$X
})
...
rlt33 <- t((sapply(p,function(x){
  apply(x,2,function(xi){
    score(xi,x[,1])
  })
}))/sapply(p.bench,function(xi){
  sapply(p.bench,function(xj){
    score(xi,xj)
  })
})[,1])
rlt33[,1] <- 0
rlt.clust <- (sort(tapply(rlt[,4],apply(apply(rlt,1,rank),2,paste,collapse=''),mean)))
rlt33 <- rlt33[match(names(sort(rlt33[,4])),rownames(rlt33)),]
rlt33 <- rbind(cbind('cluster',rlt),cbind('pheno',rlt33))
